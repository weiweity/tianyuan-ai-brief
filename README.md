...
**8 月起组合双线执行：** 客服 Agent 一期为 **P0**（设计 / G0 → 授权后开发）；供应链备案查询为 **P1**（启动级，独立门禁）。  
客服 **Ddev 未成立前** 不写产品功能代码。仓储 / 人事等只进需求池。

## 项目组合

| 板块 | 当前阶段 | 下一步 |
|------|----------|--------|
| **客服 Agent** | **P0 · 执行中 · 设计 / G0** | PRD + 08-12 demo → 08-14 G0 →（Pass）Ddev |
| **供应链 · 备案** | **P1 · 执行中 · 启动级** | 启动会 → 一刀口 + Owner + 样本/付费 → 书面启动结论 |
| 仓储 / 人事等 | 需求池 | 不排期、不占 8 月必达 |
| 2026-07-31 立项汇报 | 已归档 | 仅作历史汇报与设计方法参考 |

## 快速入口

| 你要做什么 | 入口 |
|------------|------|
| 看项目组合与 8 月执行逻辑 | [项目驾驶舱](business-docs/00-项目驾驶舱.md) |
| **看 D0 启动会结论** | **[2026-08-04 D0 纪要](business-docs/01-客服Agent项目/2026-08-04_D0启动会纪要.md)** |
| **推进客服设计** | **[客服 Agent 设计阶段](business-docs/01-客服Agent项目/20-设计-进行中/README.md)** |
