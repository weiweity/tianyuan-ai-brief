...
- Pages 发布不直接上传 `docs/`，也不复制整个 `business-docs/`。`npm run build:pages` 生成历史 Web 公开投影，并仅将通过脱敏门禁的 `09` 生成视图复制到 `/customer-agent/`；`07` PRD 与 `08` 执行中心仍统一改到不含项目内容的 `internal-only.html`。公开 `09` 在构建时必须再次验证 safe-view 标记、内嵌品牌资源和禁区扫描；人员、审批原话、协作链接、费用 cap、业务证据、PRD、Hub 与源文件哈希均不得进入公开发布包。发布副本中的 Markdown 若指向公开 Web 根以外，一律降级为不可点击的内部资料说明；构建完成前递归检查全部 HTML `href` 与 Markdown 相对链接。`npm run serve` 先生成并托管真实公开投影，避免本机可用、线上越权或断链。
...
- Mermaid 固定为 10.9.8 并随站点发布，不依赖 CDN。
- DOMPurify 固定为 3.4.13；远端 JSON、本机草稿、编辑器富文本和 Mermaid SVG 都在进入 DOM 前经过显式白名单。
...
# Pages 由 .github/workflows/pages.yml 在全量质量门禁通过后发布 dist/pages；根入口为历史回看页，/customer-agent/ 为脱敏启动会主屏，PRD 与执行中心仍只显示内部访问提示
```
