...
> **状态：** 架构设计关 **PASS-WITH-CONDITIONS（静态设计）**；实现设计关 **Pass · 文档包 Ready** · 2026-09-03 · **v1.30 · G0 / Ddev 已 Pass，DEV-M0 进行中且 W0～W4 已完成；CR-002、CR-003、CR-004、DEC-042 与扩展治理已纳入当前状态投影**\
...
**需求分析关通过 → 架构设计关 PASS-WITH-CONDITIONS（含 CR-002、CR-003、CR-004、DEC-042 与扩展治理静态增量）→ 实现设计关 Pass · 文档包 Ready（46）→ G0 / Ddev 已 Pass → DEV-M0 进行中（W0～W4 已完成）。**\
不是运行时已上线；是**技术设计与组织授权已收口，代码开发已进入 DEV-M0，W4 不可变 migration / PostgreSQL 15 控制面已验证，W5 runtime adapter / service readiness 待单独开工授权**。

这里的“静态”是证据标签：图、合同和不变量已完成设计态闭环；真实数据库、身份、代码、压测、恢复和真机证据按瀑布关卡在实现/测试/发布阶段补齐，不把后续证据倒灌成架构设计未通过。

**2026-08-21 PG 证据校正：** ENG-T1 修正后的当前 `schema.v1.12` clean-install reference DDL（SHA-256 `47b667958e522a28df1c04d7c79a56c930bfe0ac04598321824b55744ac4a801`）已在本机隔离 PostgreSQL 15.18 通过 40 tables / 2 views / 143 functions clean-install、ACL 8/8、约束 3/3、ACK runtime wrapper 正向/幂等/异体冲突、完整幂等重跑与 COMMIT 前故障原子回滚，结果 **PASS-WITH-LIMITATION**，证据 `EVD-PG15-LOCAL-PREFLIGHT-20260821T212715+0800-47B66795`。这份前置证据本身不证明 immutable migration / N/N-1 / application runtime / managed PG / backup-restore / concurrency-deadlock / production。2026-09-03 产品仓 W4 已用同一 DDL SHA 另行实现 `0001..0009`、私有账本、同会话 advisory lock / 事务与精确 catalog 后验，并在隔离 PG15 通过 N-only；真实 N-1 因无前一签名基线为 `N/A`。API / desktop 接库、application runtime、managed PG、backup-restore、concurrency-deadlock 与 production 仍 **NOT_CERTIFIED / NOT_IMPLEMENTED**。G0、Scope 与 Ddev 不因代码落地自动变化。
...
| **2 架构设计** | **PASS-WITH-CONDITIONS（静态设计）** | SSOT 37 + API 39 + DDL 33 + 图 40 + NFR 41 + CR-002 47/48 + CR-003 49/50 + CR-004 跨合同硬门 + DEC-042 内容资产治理；N/N-1、PlatformAdapter、迁移矩阵、来源绑定及 DEC-042 ID/scope/taxonomy/review/placeholder/quality/hash 已冻结为人读与静态机器合同；postfix 进一步锁定 promoted role 入 hash、固定 ASCII 键集+C 排序、population manifest、semantic asset 退役墓碑、完整 snapshot+客户端半开过滤、封闭最小 public mapper 与审计原因分流。当前 schema.v1.12 reference DDL local PG15 preflight 已 `PASS-WITH-LIMITATION`；W2 生成类型/validator、W3 `/health` 骨架与 W4 `0001..0009` migration / PG15 控制面已落地，W4 N-only 已通过且真实 N-1 为 `N/A`。application business runtime / managed PG / backup-restore / concurrency-deadlock / production 仍 `NOT_CERTIFIED / NOT_IMPLEMENTED` | 维持已锁定合同；W5 建立 runtime adapter / service readiness，仍不跨入真实数据或生产能力 |
...
| **组织授权门（不计入八关）** | **Pass · G0 / Ddev 已签发** | 外部责任包 14/14、Scope 15/15；G0 与 Ddev 分别由 `EVD-G0-SIGN-20260831`、`EVD-DDEV-AUTH-20260831` 签发 | 保持后续能力门；当前只即时放行 DEV-M0 |
| **4 代码开发** | **DEV-M0 · In progress（W0～W4 complete）** | `DEC-DDEV-01=PASS` 已授权在 `development / test` 使用合成数据进入 DEV-M0；W0 / W1 完成 workspace 与桌面机械迁移，W2 完成合同 codegen / runtime validation，W3 完成配置拒启与 loopback `/health` 骨架，W4 完成九段不可变 migration、私有账本、同会话锁 / 事务与隔离 PG15 后验；PR #10 / #11 / #13 已合并 | 单独授权 W5 runtime adapter / service readiness；DEV-M0 退出证据未齐不得进入 DEV-M1 |
| **5 单元测试** | **Not started** | W2～W4 切片级回归已通过 555 个 Vitest tests + 3 个 compiled-package smoke；W4 另通过 18 个 database unit、10 个隔离 PG15 integration 与 package smoke。这些是代码开发内验证，不等于技术第 5 关已独立进入或完成 | W5 继续补 service readiness / repository 单测；待代码开发出口后独立进关 |
...
2. **开发授权：** 当前只允许进入正式 `DEV-M0`；退出证据未齐不得进入 `DEV-M1`。\
...
4. **Ddev 后的 DEV-M0：** `W0`～`W4` 已完成；W2 生成物已匹配锁定的 schema v1.12 / OpenAPI 1.11.0，W3 只开放 loopback `/health`，W4 已完成 DDL 来源锁、`status → plan → apply → verify`、clean install、私有账本、ACL / SQLSTATE、失败回滚与 N-only 隔离 PG15 证据；真实 N-1 因无前一签名基线为 `N/A`。下一动作是单独授权 W5 runtime adapter / service readiness；未授权前不创建 API / desktop 数据库连接、不注册 `/ready` 或业务路由，不退出 DEV-M0、不进入 DEV-M1。
...
| R9 | G0-09 的单工作簿治理证据与 W4 migration / ACL 后验已齐，但 CR-004 业务运行硬门、真实飞书接入与生产 ACL 尚未实现 | G0-09 / Scope #9 的 Pass 只关闭开发前来源治理；W5+ 仍须实现并验证 API / repository / 客户端 runtime，真实接入前执行 `PROD-ACL-01` |
| R10 | DEC-042 人读与静态 machine schema/wire 已锁；W2 完成生成类型/validator，W3 完成 `/health` 骨架，W4 完成 migration / PG15 控制面；搜索/发布业务 runtime、placeholder 客户端与动态业务负例尚无 | 46/48/50 已冻结 DEV-M0～M3 任务与退出负例；后续仍需在业务运行链验证完整 snapshot 的客户端 `[from,to)` 边界、public mapper 零内部字段泄漏，以及来源 denial 与标准审计分流；未取得运行 EVD 前不宣传能力 |
...
| DEC-042 内容资产治理 | **25 / 26 / 31 / 37 / 40 / 46 / 48 / 50 + 33 / OpenAPI**（含 promoted role 治理 hash、固定 ASCII+C 排序、population manifest、semantic 退役墓碑、完整 snapshot+客户端半开过滤、封闭最小 public mapper 与审计分流；W2 已完成生成类型/validator，W3 只完成 `/health` 骨架，W4 已完成 migration / PG15 控制面；业务服务端客户端代码 / 动态 EVD 未实现） |
