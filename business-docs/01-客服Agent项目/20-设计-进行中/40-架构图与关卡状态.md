...
> **状态：** 架构设计关 **PASS-WITH-CONDITIONS（静态设计）**；实现设计关 **Pass · 文档包 Ready** · 2026-09-03 · **v1.31 · G0 / Ddev 已 Pass，DEV-M0 与 W0～W6 已完成；CR-002、CR-003、CR-004、DEC-042 与扩展治理已纳入当前状态投影**\
...
**需求分析关通过 → 架构设计关 PASS-WITH-CONDITIONS（含 CR-002、CR-003、CR-004、DEC-042 与扩展治理静态增量）→ 实现设计关 Pass · 文档包 Ready（46）→ G0 / Ddev 已 Pass → DEV-M0 与 W0～W6 已完成。**\
不是运行时已上线；是**技术设计与组织授权已收口，代码开发中的 DEV-M0 已完成，下一动作是 DEV-M1 开工评审与独立授权**。
...
| **2 架构设计** | **PASS-WITH-CONDITIONS（静态设计）** | SSOT 37 + API 39 + DDL 33 + 图 40 + NFR 41 + CR-002 47/48 + CR-003 49/50 + CR-004 跨合同硬门 + DEC-042 内容资产治理；N/N-1、PlatformAdapter、迁移矩阵、来源绑定及 DEC-042 ID/scope/taxonomy/review/placeholder/quality/hash 已冻结。W2～W6 已完成生成类型/validator、loopback `/health`、`0001..0009` migration / PG15 控制面、基础设施 `/ready`、统一 CI 与非部署候选产物边界；真实 N-1 为 `N/A`。business runtime / desktop adapter / managed PG / backup-restore / production 仍 `NOT_CERTIFIED / NOT_IMPLEMENTED` | 维持已锁定合同；DEV-M1 另行评审与授权 |
...
| **4 代码开发** | **DEV-M0 · Complete（W0～W6 complete）** | W0～W6 已完成 workspace、桌面机械迁移、合同 codegen/runtime validation、API/config、migration/PG15、runtime pool/`/ready` 与退出 CI/候选产物边界；PR #15 CI run `33729754086` 三条 lane 全绿，合并头 `44b863d` | `DEV-M1` 开工评审与独立授权；授权前不实现业务九端口或真实 adapter |
| **5 单元测试** | **Not started** | DEV-M0 代码内验证已通过完整 `pnpm check`、PG15 database 10/10、API integration 18/18、desktop 508 tests 与 Windows feasibility smoke。这些是 DEV-M0 退出证据，不等于技术第 5 关已独立进入或完成 | 待 DEV-M1 开工决定后按影响面进入独立测试关 |
...
2. **开发授权：** `DEV-M0` 已完成；`DEV-M1` 仍须独立评审与授权。\
...
4. **Ddev 后的 DEV-M0：** `W0`～`W6` 已完成；W5 已建立本机 runtime pool、service repository 与失败关闭 `/ready`，W6 已用统一 CI、Windows feasibility smoke 和非部署候选产物后验关闭退出证据；真实 N-1 因无前一签名基线为 `N/A`。下一动作是 `DEV-M1` 开工评审与独立授权；授权前不实现业务九端口、desktop adapter、真实数据或 Pilot。
...
| R9 | G0-09 的单工作簿治理证据、W4 migration / ACL 后验与 W5 基础设施 readiness 已齐，但 CR-004 业务运行硬门、真实飞书接入与生产 ACL 尚未实现 | G0-09 / Scope #9 的 Pass 只关闭开发前来源治理；DEV-M1+ 仍须实现并验证业务 API / repository / 客户端 runtime，真实接入前执行 `PROD-ACL-01` |
| R10 | DEC-042 人读与静态 machine schema/wire 已锁；W2～W6 完成 DEV-M0 基础合同与退出证据，搜索/发布业务 runtime、placeholder 客户端与动态业务负例尚无 | 46/48/50 已冻结 DEV-M0～M3 任务与退出负例；后续仍需在业务运行链验证完整 snapshot 的客户端 `[from,to)` 边界、public mapper 零内部字段泄漏，以及来源 denial 与标准审计分流；未取得运行 EVD 前不宣传能力 |
...
| DEC-042 内容资产治理 | **25 / 26 / 31 / 37 / 40 / 46 / 48 / 50 + 33 / OpenAPI**（含 promoted role 治理 hash、固定 ASCII+C 排序、population manifest、semantic 退役墓碑、完整 snapshot+客户端半开过滤、封闭最小 public mapper 与审计分流；W2～W6 已完成 DEV-M0 基础合同与退出证据；业务九端口、desktop adapter / 动态 EVD 未实现） |
