...
> **状态：** 架构设计关 **PASS-WITH-CONDITIONS（静态设计）**；实现设计关 **Pass · 文档包 Ready** · 2026-08-31 · **v1.28 · G0 / Ddev 已 Pass，DEV-M0 进行中且 W0、W1 已完成；CR-002、CR-003、CR-004、DEC-042 与扩展治理已纳入当前状态投影**\
...
**需求分析关通过 → 架构设计关 PASS-WITH-CONDITIONS（含 CR-002、CR-003、CR-004、DEC-042 与扩展治理静态增量）→ 实现设计关 Pass · 文档包 Ready（46）→ G0 / Ddev 已 Pass → DEV-M0 进行中（W0、W1 已完成）。**\
不是运行时已上线；是**技术设计与组织授权已收口，代码开发已进入 DEV-M0，下一能力待单独授权**。
...
| **4 代码开发** | **DEV-M0 · In progress（W0、W1 complete）** | `DEC-DDEV-01=PASS` 已授权在 `development / test` 使用合成数据进入 DEV-M0；产品仓已完成 workspace scaffold 与 desktop mechanical move，证据 `EVD-DEV-M0-W0-20260831`、`EVD-DEV-M0-W1-20260831` | 单独授权合同开发与 codegen / runtime validation；DEV-M0 退出证据未齐不得进入 DEV-M1 |
| **5 单元测试** | **Not started** | W0 / W1 已运行的最高 506 项测试属于原型回归、scaffold 与机械迁移行为等价证据，不等于技术第 5 关已进入或完成 | 随 DEV-M0 正式模块补齐运行时单测，待代码开发出口后独立进关 |
