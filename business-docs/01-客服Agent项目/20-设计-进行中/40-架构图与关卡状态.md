...
> **状态：** 架构设计关 **PASS-WITH-CONDITIONS（静态设计）**；实现设计关 **Pass · 文档包 Ready** · 2026-09-02 · **v1.29 · G0 / Ddev 已 Pass，DEV-M0 进行中且 W0～W3 已完成；CR-002、CR-003、CR-004、DEC-042 与扩展治理已纳入当前状态投影**\
...
**需求分析关通过 → 架构设计关 PASS-WITH-CONDITIONS（含 CR-002、CR-003、CR-004、DEC-042 与扩展治理静态增量）→ 实现设计关 Pass · 文档包 Ready（46）→ G0 / Ddev 已 Pass → DEV-M0 进行中（W0～W3 已完成）。**\
不是运行时已上线；是**技术设计与组织授权已收口，代码开发已进入 DEV-M0，W4 不可变 migration / PostgreSQL 深模块待单独开工授权**。
...
| **4 代码开发** | **DEV-M0 · In progress（W0～W3 complete）** | `DEC-DDEV-01=PASS` 已授权在 `development / test` 使用合成数据进入 DEV-M0；W0 / W1 完成 workspace 与桌面机械迁移，W2 完成合同 codegen / runtime validation，W3 完成配置拒启与 loopback `/health` 骨架；PR #10 / #11 已合并 | 单独授权 W4 不可变 migration / PostgreSQL 深模块；DEV-M0 退出证据未齐不得进入 DEV-M1 |
| **5 单元测试** | **Not started** | W2 / W3 切片级合同、配置、API 与桌面回归已通过 537 个 Vitest tests + 2 个 compiled-package smoke；这些是代码开发内验证，不等于技术第 5 关已独立进入或完成 | W4 随不可变 migration / PG 补齐运行时单测；待代码开发出口后独立进关 |
...
4. **Ddev 后的 DEV-M0：** `W0`～`W3` 已完成；W2 生成物已匹配锁定的 schema v1.12 / OpenAPI 1.11.0，W3 只开放 loopback `/health`。下一动作是单独授权 W4 不可变 migration / PostgreSQL 深模块，完成 DDL 来源锁、`status → plan → apply → verify`、empty install、来源与内容治理硬门负例、backfill、ACL / SQLSTATE、失败回滚及 N-only / N-1 兼容证据。首版没有真实 N-1 必须诚实记 `N/A`；未绿不注册业务路由、不退出 DEV-M0、不进入 DEV-M1。
...
| R9 | G0-09 的单工作簿治理证据已齐，但 CR-004 运行硬门、真实飞书接入与生产 ACL 尚未实现 | G0-09 / Scope #9 的 Pass 只关闭开发前来源治理；Ddev 后仍须实现并验证迁移、API / PG / 客户端 runtime，真实接入前执行 `PROD-ACL-01` |
| R10 | DEC-042 人读与静态 machine schema/wire 已锁；W2 只完成生成类型/validator，W3 只完成 `/health` 骨架，迁移、搜索/发布业务 runtime、placeholder 客户端与动态负例尚无 | 46/48/50 已冻结 DEV-M0～M3 任务与退出负例；需验证固定 ASCII+C 排序且 role 变更必改 hash、同计数换行必改 population manifest、semantic 先退役留墓碑再删 query、完整 snapshot 的客户端 `[from,to)` 边界、public mapper 零内部字段泄漏，以及来源 denial 与标准审计分流；未取得运行 EVD 前不宣传能力 |
...
| DEC-042 内容资产治理 | **25 / 26 / 31 / 37 / 40 / 46 / 48 / 50 + 33 / OpenAPI**（含 promoted role 治理 hash、固定 ASCII+C 排序、population manifest、semantic 退役墓碑、完整 snapshot+客户端半开过滤、封闭最小 public mapper 与审计分流；W2 已完成生成类型/validator，W3 只完成 `/health` 骨架，迁移 / PostgreSQL / 业务服务端客户端代码 / 动态 EVD 未实现） |
