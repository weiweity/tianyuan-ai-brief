...
已验证：隔离 PG15 安装、PKCE 错 verifier、登录重复消费/撤销与未撤销会话隔离、审核回执、角色与跨能力拒绝（含跨函数 EXECUTE）、分页投影、质量覆盖/质量门、缺陷阻断后拒绝恢复、质量幂等冲突、异人审核缺证据拒绝、恢复/取消，以及两个连接的内容锁互斥。静态及这些针对性行为检查不代表完整端到端发布、501+ 扩样负载、所有并发恢复矩阵或真实身份运行已验收。合同仍为 DRAFT，未冻结。
