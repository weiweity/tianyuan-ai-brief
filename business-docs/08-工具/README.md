...
| `build_customer_agent_backend_contract.mjs` | 从固定 owner 基线及候选增量生成 OpenAPI 1.13 / schema.v1.16；`--write` 生成、`--check` 校验；不覆盖旧版本 |
...
2. `DEC-058` 下的 G0-03 只冻结开发问题假设：明确话术检索推荐与工单只读分析两域；无法可靠取得的频次、耗时、错误与经营指标统一写 `NOT_MEASURED`，真实坐席与真实分母后移 G1a / G1b。
3. `DEC-058` 下的 G0-13 只冻结产品仓可执行的纯合成 `20 + 12 + 18 = 50` 条开发合同；它不得填入 `03` 的真实 G1a 结果表。真实内容快照、独立盲审、分层覆盖和关键词 / 候选对照在 G1a 另建。
4. 先向用户展示列映射、缺口和将要计算的指标，经一次确认后再计算；输出只回填脱敏聚合值、版本与 `EVD-*` 收据，不回填逐行原始数据或可逆哈希。
5. 当前开发前证据分别为 `EVD-G0-03-MENOKIN-APPLICABILITY-20260830` 与 `EVD-G0-13-MENOKIN-EVALUATION-FREEZE-20260830`；二者仍分账。独立盲审不是开发前证据，但仍是 G1a 硬门。
...
`DEC-058 / DEC-062` 允许同一企业工作簿的四个子表共用一个 `SRC-*`、一个物理 `WBV-*`、一个脱敏快照 / 质量证据、一个 ACL 证据和一个整体批准证据；`presale / campaign / aftersale / product` 四个逻辑域分别使用独立 `srcv_*`，以满足 CR-004 注册表的主键与 domain 约束。当前收口证据为 `EVD-G0-09-AUTHORITY-SOURCES-20260830`。

- 无业务语义的公开 `SRC-*`、物理 `WBV-*` 与四个独立 `srcv_*` 代号，以及它们到真实飞书工作簿、版本和快照的受控私有映射；
- `canonical / current` 结论、平台 / 商品 / 活动适用范围、生效 / 失效 / 复核时间；活动类必须有完整双边时间；
- L1 单人 Owner ACL 证据；企业级继承、多人职责分离和应用权限在真实接入前由 `PROD-ACL-01` 复核；
- 总行数、缺字段、重复、冲突、过期、敏感命中、可导入和隔离数量，所有质量数必须带分母；
- 冲突或未修项的隔离 / 裁决结论，以及 Content Lead 对四域整体清单的最终批准。

归档前先做脱敏预览；公开仓只接收不透明代号、脱敏聚合、单向内容摘要与最终 `EVD-*`，不接收飞书 URL / token / 资源标题、真实 revision、导出文件、成员清单或内容正文。只有四行全部为 `READY`、分母守恒、共享值一致且 Content Lead 已签发时，才允许用同一证据关闭 G0-09 与 Scope #9。

现行脱敏投影为 `01-客服Agent项目/90-评审/2026-08-30_G0-09单工作簿脱敏快照.json`。状态解析器校验四域恰好各一行、代号格式、质量分母守恒、Content Lead 角色、4/4 `READY` 与单工作簿共享证据；它不会读取话术正文或代签 G0。
