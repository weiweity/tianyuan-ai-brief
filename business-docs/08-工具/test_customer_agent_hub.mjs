...
await check("HTML 文件存在且为只读生成视图", async () => {
  ...
  if (mode === "public-template") {
    ...
    assert.match(payload.headline.summary, /DEV-M1 已完成.*实施已批准的桌面合成接入 D1–D5.*新增范围仍须专项批准/);
    assert.match(payload.headline.nowTitle, /DEV-M1 已完成.*下一动作以当前台账为准/);
    assert.match(payload.headline.nowSummary, /W0、W1、W2、W3、W4、W5 已完成.*实施已批准的桌面合成接入 D1–D5.*按专项批准范围推进.*不激活未授权 runtime/);
    assert.match(payload.headline.scheduleTitle, /DEV-M1 已完成.*实施已批准的桌面合成接入 D1–D5.*按专项批准范围推进.*单人全栈 \/ FDE/);
    assert.match(payload.meeting.title, /DEV-M1 已完成.*W0、W1、W2、W3、W4、W5 已收口.*下一动作以当前台账为准/);
    assert.match(payload.meeting.positioning, /DEV-M1 退出结果与当前下一动作复核会.*W0、W1、W2、W3、W4、W5 已收口.*实施已批准的桌面合成接入 D1–D5.*按专项批准范围推进.*CR \/ DEC/);
    assert.equal(payload.meeting.copyTitle, "客服 Agent 当前推进清单");
    assert.equal(payload.gates.every((gate) => gate.status === "Pass"), true);
    assert.equal(payload.headline.nextTitle, "当前下一动作");
    assert.match(payload.headline.nextOutput, /实施已批准的桌面合成接入 D1–D5.*按专项批准范围推进/);
    assert.doesNotMatch(JSON.stringify([payload.headline, payload.meeting, payload.prelaunchChecklist, payload.schedule[0]]), /等待下一里程碑授权|仅梳理下一动作|授权前不实施|D1–D5[^。]*尚未授权/);
    assert.equal(payload.schedule.length, 5);
    assert.equal(payload.schedule[0].title, "DEV-M1 · W0、W1、W2、W3、W4、W5 已完成");
    assert.equal(payload.schedule[0].date, "2026-08-31");
    assert.match(payload.schedule[0].action, /下一动作 实施已批准的桌面合成接入 D1–D5.*按专项批准范围推进/);
    ...
    assert.match(payload.prelaunchChecklist[0], /保持 W0、W1、W2、W3、W4、W5 基线.*行为不漂移/);
    assert.match(payload.prelaunchChecklist[1], /按当前台账推进下一动作「实施已批准的桌面合成接入 D1–D5.*」.*不得超出专项批准范围/);
    assert.match(payload.prelaunchChecklist[2], /获批后.*受影响测试.*构建.*workspace.*E2E.*实施证据/);
    assert.match(payload.prelaunchChecklist[3], /development \/ test.*合成数据.*不启用未经批准的真实数据.*后续里程碑/);
