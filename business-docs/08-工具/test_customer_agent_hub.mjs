...
await check("HTML 文件存在且为只读生成视图", async () => {
  ...
  if (mode === "public-template") {
    ...
    assert.match(payload.headline.summary, /DEV-M1 已完成.*W0、W1、W2、W3、W4、W5 已收口.*DEC-SEARCH-01 \/ 真实 G1a 准入复核.*待单独授权.*下一里程碑未授权/);
    assert.match(payload.headline.nowTitle, /DEV-M1 已完成.*等待下一里程碑授权/);
    assert.match(payload.headline.nowSummary, /W0、W1、W2、W3、W4、W5 已完成.*DEC-SEARCH-01 \/ 真实 G1a 准入复核.*待单独授权.*不激活未授权 runtime/);
    assert.match(payload.headline.scheduleTitle, /DEV-M1 已完成.*DEC-SEARCH-01 \/ 真实 G1a 准入复核.*待单独授权.*单人全栈 \/ FDE/);
    assert.match(payload.meeting.title, /DEV-M1 已完成.*W0、W1、W2、W3、W4、W5 已收口.*下一里程碑待授权/);
    assert.match(payload.meeting.positioning, /DEV-M1 退出结果与下一里程碑授权复核会.*W0、W1、W2、W3、W4、W5 已收口.*DEC-SEARCH-01 \/ 真实 G1a 准入复核.*尚未授权.*CR \/ DEC/);
    ...
    assert.match(payload.headline.nextOutput, /DEC-SEARCH-01 \/ 真实 G1a 准入复核.*授权前不实施/);
    ...
    assert.match(payload.schedule[0].action, /下一动作 DEC-SEARCH-01 \/ 真实 G1a 准入复核.*未授权前不实施/);
    ...
    assert.match(payload.prelaunchChecklist[1], /只准备DEC-SEARCH-01 \/ 真实 G1a 准入复核.*范围、验收与授权输入.*授权前不实施/);
