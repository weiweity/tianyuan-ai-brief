...
await check("HTML 文件存在且为只读生成视图", async () => {
  ...
  if (mode === "public-template") {
    ...
    assert.match(payload.headline.title, /DEV-M0 已开工.*W0、W1、W2、W3 已完成/);
    assert.match(payload.headline.summary, /DEV-M0 正在进行.*W0、W1、W2、W3 已完成.*DEV-M0-W4 不可变 migration \/ PostgreSQL 深模块.*待单独授权.*不得进入下一里程碑/);
    assert.match(payload.headline.nowTitle, /DEV-M0 进行中.*等待下一动作授权/);
    assert.match(payload.headline.nowSummary, /W0、W1、W2、W3 已完成.*DEV-M0-W4 不可变 migration \/ PostgreSQL 深模块.*待单独授权.*不激活未授权 runtime/);
    assert.match(payload.headline.scheduleTitle, /DEV-M0 已开始.*W0、W1、W2、W3 已完成.*单人全栈 \/ FDE/);
    assert.match(payload.meeting.title, /DEV-M0 已开始.*W0、W1、W2、W3 已完成.*下一动作待授权/);
    assert.match(payload.meeting.positioning, /DEV-M0 实施与证据复核会.*W0、W1、W2、W3 已完成.*DEV-M0-W4 不可变 migration \/ PostgreSQL 深模块.*尚未授权.*CR \/ DEC/);
    ...
    assert.match(payload.headline.nextOutput, /DEV-M0-W4 不可变 migration \/ PostgreSQL 深模块.*授权前不实施/);
    assert.equal(payload.schedule.length, 5);
    assert.equal(payload.schedule[0].title, "DEV-M0 · W0、W1、W2、W3 已完成");
    assert.equal(payload.schedule[0].date, "2026-08-31");
    assert.match(payload.schedule[0].action, /下一动作 DEV-M0-W4 不可变 migration \/ PostgreSQL 深模块.*未授权前不实施/);
    ...
    assert.match(payload.prelaunchChecklist[0], /保持 W0、W1、W2、W3 基线.*行为不漂移/);
    assert.match(payload.prelaunchChecklist[1], /只准备DEV-M0-W4 不可变 migration \/ PostgreSQL 深模块.*范围、验收与授权输入.*授权前不实施/);
