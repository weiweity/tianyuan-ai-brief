...
await check("HTML 文件存在且为只读生成视图", async () => {
  ...
  if (mode === "public-template") {
    ...
    assert.match(payload.headline.title, /DEV-M0 已完成.*W0、W1、W2、W3、W4、W5、W6 已收口/);
    assert.match(payload.headline.summary, /DEV-M0 已完成.*W0、W1、W2、W3、W4、W5、W6 已收口.*DEV-M1 开工评审与授权.*待单独授权.*下一里程碑未授权/);
    assert.match(payload.headline.nowTitle, /DEV-M0 已完成.*等待下一里程碑授权/);
    assert.match(payload.headline.nowSummary, /W0、W1、W2、W3、W4、W5、W6 已完成.*DEV-M1 开工评审与授权.*待单独授权.*不激活未授权 runtime/);
    assert.match(payload.headline.scheduleTitle, /DEV-M0 已完成.*DEV-M1 开工评审与授权.*待单独授权.*单人全栈 \/ FDE/);
    assert.match(payload.meeting.title, /DEV-M0 已完成.*W0、W1、W2、W3、W4、W5、W6 已收口.*下一里程碑待授权/);
    assert.match(payload.meeting.positioning, /DEV-M0 退出结果与下一里程碑授权复核会.*W0、W1、W2、W3、W4、W5、W6 已收口.*DEV-M1 开工评审与授权.*尚未授权.*CR \/ DEC/);
    assert.equal(payload.meeting.copyTitle, "客服 Agent 当前推进清单");
    assert.equal(payload.gates.every((gate) => gate.status === "Pass"), true);
    assert.equal(payload.headline.nextTitle, "下一里程碑授权");
    assert.match(payload.headline.nextOutput, /DEV-M1 开工评审与授权.*授权前不实施/);
    assert.equal(payload.schedule.length, 5);
    assert.equal(payload.schedule[0].title, "DEV-M0 · W0、W1、W2、W3、W4、W5、W6 已完成");
    assert.equal(payload.schedule[0].date, "2026-08-31");
    assert.match(payload.schedule[0].action, /下一动作 DEV-M1 开工评审与授权.*未授权前不实施/);
    ...
    assert.match(payload.prelaunchChecklist[0], /保持 W0、W1、W2、W3、W4、W5、W6 基线.*行为不漂移/);
    assert.match(payload.prelaunchChecklist[1], /只准备DEV-M1 开工评审与授权.*范围、验收与授权输入.*授权前不实施/);
