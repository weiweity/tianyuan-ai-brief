...
await check("HTML 文件存在且为只读生成视图", async () => {
  ...
  if (mode === "public-template") {
    ...
    assert.match(payload.headline.summary, /DEV-M1 已完成.*T4 BLOCKED · T5 ATTEMPTED \/ BLOCKED.*33 条逐条风险审定与版本化新包准备.*下一里程碑未授权/);
    assert.match(payload.headline.nowTitle, /DEV-M1 已完成.*等待下一里程碑授权/);
    assert.match(payload.headline.nowSummary, /W0、W1、W2、W3、W4、W5 已完成.*33 条逐条风险审定与版本化新包准备.*待单独授权.*不激活未授权 runtime/);
    assert.match(payload.headline.scheduleTitle, /DEV-M1 已完成.*33 条逐条风险审定与版本化新包准备.*待单独授权.*单人全栈 \/ FDE/);
    assert.match(payload.meeting.title, /DEV-M1 已完成.*W0、W1、W2、W3、W4、W5 已收口.*下一里程碑待授权/);
    assert.match(payload.meeting.positioning, /DEV-M1 退出结果与下一里程碑授权复核会.*W0、W1、W2、W3、W4、W5 已收口.*33 条逐条风险审定与版本化新包准备.*尚未授权.*CR \/ DEC/);
    ...
    assert.match(payload.headline.nextOutput, /33 条逐条风险审定与版本化新包准备.*授权前不实施/);
    ...
    assert.match(payload.schedule[0].action, /下一动作 33 条逐条风险审定与版本化新包准备.*未授权前不实施/);
    ...
    assert.match(payload.prelaunchChecklist[1], /只准备33 条逐条风险审定与版本化新包准备.*范围、验收与授权输入.*授权前不实施/);
