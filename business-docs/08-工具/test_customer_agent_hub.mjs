...
await check("HTML 文件存在且为只读生成视图", async () => {
  ...
  if (mode === "public-template") {
    ...
    assert.match(payload.meeting.positioning, /DEV-M1 退出结果与下一里程碑授权复核会.*W0、W1、W2、W3、W4、W5 已收口.*T5 最终 dry-run 与独立运行授权.*尚未授权.*CR \/ DEC/);
    ...
    assert.match(payload.headline.nextOutput, /T5 最终 dry-run 与独立运行授权.*授权前不实施/);
    ...
    assert.match(payload.schedule[0].action, /下一动作 T5 最终 dry-run 与独立运行授权.*未授权前不实施/);
    ...
    assert.match(payload.prelaunchChecklist[1], /只准备T5 最终 dry-run 与独立运行授权.*范围、验收与授权输入.*授权前不实施/);
