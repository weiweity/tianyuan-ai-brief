...
test("DEC-058 单人开发前证据保持四域完整且不携带原始内容", async () => {
  ...
  assert.equal(Object.hasOwn(snapshot, "workbook_sha256"), false);
  ...
  assert.doesNotMatch(snapshotText, /\b[0-9a-f]{64}\b/i);
  ...
  assert.doesNotMatch(workbookClosure, /\b[0-9a-f]{64}\b/i);
  assert.doesNotMatch(workbookClosure, /workbook_sha256|内容摘要 SHA-256/i);
