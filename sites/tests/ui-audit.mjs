...
async function runHistoricalCurrentNavigationAudit(browser) {
  ...
  const hubStatusPattern =
    /DEV-M1 已完成，W0、W1、W2、W3、W4、W5 已收口[\s\S]*33 条逐条风险审定与版本化新包准备[\s\S]*待单独授权[\s\S]*下一里程碑未授权/;
  const prdStatusPattern =
    /G0 (?:\/|与) Ddev[^。\n]*签发[\s\S]*DEV-M1 产品实施(?:与退出证据)?已完成[\s\S]*33 条逐条风险审定与版本化新包准备[\s\S]*仍须单独授权[\s\S]*仍未部署/i;
