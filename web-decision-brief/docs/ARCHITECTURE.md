...
> 生命周期：**2026-07-31 已收尾 · 非当前客服 Agent 项目 SSOT**；现行 PRD 与执行中心在 `business-docs/01-客服Agent项目/`
...
- `/`：历史汇报回看入口。首屏固定展示“历史快照 · 非当前 SSOT”及现行客服 PRD / 执行中心入口；作者工具和 E / S 快捷键不可见、不可触发。
- Pages 发布不直接上传 `docs/`，也绝不复制 `business-docs/`。`npm run build:pages` 只生成历史 Web 的公开投影，并在 artifact 内把两个现行材料入口统一改到不含项目内容的 `internal-only.html`；人员、审批原话、协作链接、费用 cap、业务证据和源文件哈希不得进入公开发布包。源码仍保留仓库内相对入口，供授权环境下通过 `file://` 访问。发布副本中的 Markdown 若指向公开 Web 根以外，一律降级为不可点击的内部资料说明；构建完成前递归检查全部 HTML `href` 与 Markdown 相对链接。`npm run serve` 先生成并托管真实公开投影，避免本机可用、线上越权或断链。
